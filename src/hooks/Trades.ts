...
function generateAllRoutePairs(
  tokenA?: Token,
  tokenB?: Token,
  chainId?: ChainId
): [Token, Token][] {
  ...
  const customBasesA =
    customBases && tokenA ? customBases[tokenA.address] ?? [] : [];
  const customBasesB =
    customBases && tokenB ? customBases[tokenB.address] ?? [] : [];
  ...
  return [
    ...
    .filter(([tokenA, tokenB]) => {
      ...
      const restrictedBasesA: Token[] | undefined =
        restrictedBases[tokenA.address];
      const restrictedBasesB: Token[] | undefined =
        restrictedBases[tokenB.address];

      if (!restrictedBasesA && !restrictedBasesB) return true;

      if (
        restrictedBasesA &&
        !restrictedBasesA.find((base) => tokenB.equals(base))
      )
        return false;
      if (
        restrictedBasesB &&
        !restrictedBasesB.find((base) => tokenA.equals(base))
      )
        return false;
...
function useAllCommonPairs(currencyA?: Currency, currencyB?: Currency): Pair[] {
  ...
  const allPairCombinations: [Token, Token][] = useMemo(
    () => generateAllRoutePairs(tokenA, tokenB, chainId),
